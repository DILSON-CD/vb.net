...
Partial Class Pledge_SMA_classification_SMA_Areawise
    ...
    Dim tot_glwt, tot_glos, sma0_amt, sma1_amt, sma2_amt, sma3_amt, npa_per, tot_per, tot_lncnt, sma0_cnt, sma1_cnt, sma2_cnt, sma3_cnt, nodue_cnt, nodue_amt, nodue_per, due_cnt, due_amt, due_per, non_npa_cnt, non_npa_amt, tot_npa_per, tot_npa_amt As Double
    Dim reg_id As Integer
    Dim fnm, fdt, tdt As String
    Dim emp As Array

    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        Dim usr As String = Session("user_id")
        reg_id = Request.QueryString("reg_id")
    ...
    Sub FillReportHeader()
        ...
        tc31.Text = "<font color='#663366' size=2><b>AREAWISE SMA REPORT AS ON  " & fdt & "</font></b>"
    ...
    Sub FillColumnHeader()
        ...
        Dim tc51, tc52, tc53, tc54, tc55, tc56, tc57, tc58, tc59, tc510, tc511, tc512, tc513, tc514 As New TableCell
        ...
        tc51.Text = "<font size=2><b>AREA</font></b>"
        ...
        Dim tc61, tc62, tc63, tc64, tc65, tc66, tc67, tc68, tc69, tc610, tc611, tc612, tc613, tc614, tc615, tc616, tc617, tc618, tc619, tc620, tc621, tc622, tc623 As New TableCell
        ...
        tc622.Text = "<font size=2><b>Amt-Cr</font></b>"
        tc623.Text = "<font size=2><b>%</font></b>"
    ...
    Sub FillColumnField()
        ...
        If fdt = tdt Then
            sql = "select b.area_name, round(sum(S.COL_GL_WT) / 1000, 3) GL_WEIGHT, sum(s.col_total_ln_cnt) PLEDGE_COUNT, round(sum(s.col_total_ln_bal) / 10000000, 3) PLEDGE_AMOUNT, sum(s.col_nodue_ln_cnt) NO_DUE_COUNT, round(sum(s.col_nodue_ln_bal) / 10000000, 3) NO_DUE_AMOUNT, round((sum(s.col_nodue_ln_bal) / 10000000) /(sum(s.col_total_ln_bal) / 10000000) * 100, 2) NO_DUE_PERCENTAGE, sum(nvl(s.col_sma0_ln_cnt, 0) + nvl(s.col_sma1_ln_cnt, 0) + nvl(s.col_sma2_ln_cnt, 0) + nvl(s.col_sma3_ln_cnt, 0)) DUES_COUNT, round(sum(nvl(s.col_sma0_ln_bal / 10000000, 0) + nvl(s.col_sma1_ln_bal / 10000000, 0) + nvl(s.col_sma2_ln_bal / 10000000, 0) + nvl(s.col_sma3_ln_bal / 10000000, 0)), 3) DUES_AMOUNT, round((sum(nvl(s.col_sma0_ln_bal / 10000000, 0) + nvl(s.col_sma1_ln_bal / 10000000, 0) + nvl(s.col_sma2_ln_bal / 10000000, 0) + nvl(s.col_sma3_ln_bal / 10000000, 0))) / (sum(s.col_total_ln_bal) / 10000000) * 100, 2) DUES_PERCENTAGE, sum(s.col_sma0_ln_cnt) SMA0_COUNT, round(sum(s.col_sma0_ln_bal) / 10000000, 3) SMA0_AMOUNT, sum(s.col_sma1_ln_cnt) SMA1_COUNT, round(sum(s.col_sma1_ln_bal) / 10000000, 3) SMA1_AMOUNT, sum(s.col_sma2_ln_cnt) SMA2_COUNT, round(sum(s.col_sma2_ln_bal) / 10000000, 3) SMA2_AMOUNT, sum(s.col_sma3_ln_cnt) SMA3_COUNT, round(sum(s.col_sma3_ln_bal) / 10000000, 3) SMA3_AMOUNT, round((sum(s.col_sma3_ln_bal) / 10000000) / (sum(s.col_total_ln_bal) / 10000000) * 100, 2) NPA_PERCENTAGE, b.area_id,nvl(sum(s.col_nonnpa_cnt),0) NONNPA_COUNT, nvl(round(sum(s.col_nonnpa_bal) / 10000000, 2),0) NONNPA_BALANCE,round((sum(s.col_sma3_ln_bal) / 10000000) + nvl(sum(s.col_nonnpa_bal) / 10000000,0),2) TOTAL,round((nvl((sum(s.col_sma3_ln_bal) / 10000000),0) + nvl(sum(s.col_nonnpa_bal) / 10000000,0))/(sum(s.col_total_ln_bal) / 10000000) * 100,2) TOTAL_PERCENTAGE from tbl_sma_classification_branch_dtl s,branch_dtl_new b,tbl_fzm_master f where s.col_brid=b.BRANCH_ID and b.reg_id=f.region_id and b.reg_id=" & reg_id & " group by b.area_name,b.area_id order by b.area_name"
            dt = oh.ExecuteDataSet(sql).Tables(0)
        Else
            sql = "select b.area_name, round(sum(S.COL_GL_WT) / 1000, 3) GL_WEIGHT, sum(s.col_total_ln_cnt) PLEDGE_COUNT, round(sum(s.col_total_ln_bal) / 10000000, 3) PLEDGE_AMOUNT, sum(s.col_nodue_ln_cnt) NO_DUE_COUNT, round(sum(s.col_nodue_ln_bal) / 10000000, 3) NO_DUE_AMOUNT, round((sum(s.col_nodue_ln_bal) / 10000000) /(sum(s.col_total_ln_bal) / 10000000) * 100, 2) NO_DUE_PERCENTAGE, sum(nvl(s.col_sma0_ln_cnt, 0) + nvl(s.col_sma1_ln_cnt, 0) + nvl(s.col_sma2_ln_cnt, 0) + nvl(s.col_sma3_ln_cnt, 0)) DUES_COUNT, round(sum(nvl(s.col_sma0_ln_bal / 10000000, 0) + nvl(s.col_sma1_ln_bal / 10000000, 0) + nvl(s.col_sma2_ln_bal / 10000000, 0) + nvl(s.col_sma3_ln_bal / 10000000, 0)), 3) DUES_AMOUNT, round((sum(nvl(nvl(s.col_sma0_ln_bal / 10000000, 0) + s.col_sma1_ln_bal / 10000000, 0) + nvl(s.col_sma2_ln_bal / 10000000, 0) + nvl(s.col_sma3_ln_bal / 10000000, 0))) / (sum(s.col_total_ln_bal) / 10000000) * 100, 2) DUES_PERCENTAGE, sum(s.col_sma0_ln_cnt) SMA0_COUNT, round(sum(s.col_sma0_ln_bal) / 10000000, 3) SMA0_AMOUNT, sum(s.col_sma1_ln_cnt) SMA1_COUNT, round(sum(s.col_sma1_ln_bal) / 10000000, 3) SMA1_AMOUNT, sum(s.col_sma2_ln_cnt) SMA2_COUNT, round(sum(s.col_sma2_ln_bal) / 10000000, 3) SMA2_AMOUNT, sum(s.col_sma3_ln_cnt) SMA3_COUNT, round(sum(s.col_sma3_ln_bal) / 10000000, 3) SMA3_AMOUNT, round((sum(s.col_sma3_ln_bal) / 10000000) / (sum(s.col_total_ln_bal) / 10000000) * 100, 2) NPA_PERCENTAGE, b.area_id,nvl(sum(s.col_nonnpa_cnt),0) NONNPA_COUNT, nvl(round(sum(s.col_nonnpa_bal) / 10000000, 2),0) NONNPA_BALANCE,round((sum(s.col_sma3_ln_bal) / 10000000) + nvl(sum(s.col_nonnpa_bal) / 10000000,0),2) TOTAL,round((nvl((sum(s.col_sma3_ln_bal) / 10000000),0) + nvl(sum(s.col_nonnpa_bal) / 10000000,0))/(sum(s.col_total_ln_bal) / 10000000) * 100,2) TOTAL_PERCENTAGE from tbl_sma_classification_branch_dtl_his s,branch_dtl_new b,tbl_fzm_master f where s.col_brid=b.BRANCH_ID and b.reg_id=f.region_id and b.reg_id=" & reg_id & " and to_date(s.col_updt_date)='" & fdt & "' group by b.area_name,b.area_id order by b.area_name"
            dt = oh.ExecuteDataSet(sql).Tables(0)
        End If
        ...
        For Each dr In dt.Rows
            ...
            tr7.Height = "20"
            Dim tc71, tc72, tc73, tc74, tc75, tc76, tc77, tc78, tc79, tc710, tc711, tc712, tc713, tc714, tc715, tc716, tc717, tc718, tc719, tc720, tc721, tc722, tc723 As New TableCell
            If color = 0 Then
            ...
            tc71.Text = "<font size=2><a href = javascript:GetBranchwiseSMA(" & dr(19) & ",'" & fdt & "','" & tdt & "') > " & dr(0) & "</a></font>" 'zone id
            ' tc72.Text = "<font size=2>" & dr(1) & "</font>" 'glwt
            tc73.Text = "<font size=2>" & dr(2) & "</font>"  'tot_cnt
            tc74.Text = "<font size=2>" & dr(3) & "</font>"  'gl o/s
            tc75.Text = "<font size=2>" & dr(4) & "</font>"  'nodue_cnt
            tc76.Text = "<font size=2>" & dr(5) & "</font>" 'nodue_glos_amt
            tc77.Text = "<font size=2>" & dr(6) & "%</font>"  'nodue_glos_per
            tc78.Text = "<font size=2>" & dr(7) & "</font>"  'due_cnt
            tc79.Text = "<font size=2>" & dr(8) & "</font>"  'due_glos_amt
            tc710.Text = "<font size=2>" & dr(9) & "%</font>"  'due_glos_per
            tc711.Text = "<font size=2>" & dr(10) & "</font>"   'sma0_cnt
            tc712.Text = "<font size=2>" & dr(11) & "</font>"  'sma0_amt
            tc713.Text = "<font size=2>" & dr(12) & "</font>"  'sma1_cnt
            tc714.Text = "<font size=2>" & dr(13) & "</font>"   'sma1_amt
            tc715.Text = "<font size=2>" & dr(14) & "</font>"  'sma2_cnt
            tc716.Text = "<font size=2>" & dr(15) & "</font>"  'sma2_amt
            tc717.Text = "<font size=2>" & dr(16) & "</font>"   'sma3_cnt
            tc718.Text = "<font size=2>" & dr(17) & "</font>"   'sma3_amt
            tc719.Text = "<font size=2>" & dr(18) & "%</font>"   'npa_per
            tc720.Text = "<font size=2>" & dr(20) & "</font>"   'non-npa-cnt
            tc721.Text = "<font size=2>" & dr(21) & "</font>"   'non-npa-amt
        ...
        tot_npa_per = ((sma3_amt + non_npa_amt) / tot_glos) * 100
    ...
    Sub FillSummaryField()
        ...
        Dim tcc1, tcc2, tcc3, tcc4, tcc5, tcc6, tcc7, tcc8, tcc9, tcc10, tcc11, tcc12, tcc13, tcc14, tcc15, tcc16, tcc17, tcc18, tcc19, tcc20, tcc21, tcc22, tcc23 As New TableCell
        tcc1.ColumnSpan = 5
        'tcc2.ColumnSpan = 2
        tcc3.ColumnSpan = 4
        tcc4.ColumnSpan = 3
        ...
        tcc22.Text = "<font size=2><b>" & tot_npa_amt & "</font></b>"
        tcc23.Text = "<font size=2><b>" & Math.Round(tot_npa_per, 2) & "%</font></b>"
